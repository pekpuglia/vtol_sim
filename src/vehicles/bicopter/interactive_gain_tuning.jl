...
# ╔═╡ 002a7848-62db-11ee-3ae5-e937a22d149e
using ControlSystems

# ╔═╡ 10617579-f5b2-4012-b171-669f4e3b4b01
using Plots

# ╔═╡ 563b4856-82eb-44dc-b75a-02545ed220cd
...
Esse notebook mostra o processo de criação e ajuste de um controlador de posição para um drone 3DoF. 

Um drone 3DoF é um modelo simplificado que considera o drone restrito a um plano vertical. Assim, temos 2 translações (x e y) e uma rotação. Consideramos que o drone tem 2 motores, um à esquerda e outro à direita. Esse modelo é um bom degrau para chegar na dinâmica 6DoF (corpo rígido) de um drone real.
"""

# ╔═╡ cded54bf-4f3c-4d83-8bf2-0447e26d0103
md"""
# Modelo e linearização

Começamos com o modelo não linear da dinâmica do drone. Consideramos como vetor de estado

``
Xᵗ = [x\; y\; θ\; xdot\; ydot\; θdot]
``

e os empuxos dos motores são representados em um vetor de entradas 

``uᵗ = [T_{esq}\; T_{dir}]``, 

cujo cálculo é o objetivo principal do controle do drone. O fato do vetor de entrada ter 2 componentes leva ao nome usual desse modelo, 'bicóptero'. Procuramos escrever a função 

``\dot{X} = f(X, u; ...)``, 

onde "..." representam parâmetros físicos que descrevem o drone. Um breve gagá de diagrama de corpo livre mostra que precisamos da inércia rotacional do drone, sua massa, a gravidade local e a largura do drone (distância entre os motores).
"""
...
function bicopter_dynamics(X::Vector, u::Vector, p::Bicopter)
    [
        X[4:end]
        -(u[1] + u[2]) * sin(X[3]) / p.mass
        (u[1] + u[2]) * cos(X[3]) / p.mass - p.gravity
        p.prop_dist * (u[2] - u[1]) / p.inertia
    ]
end

# ╔═╡ f940fbbe-2c72-4dde-b818-adba968b3753
md"""
## Linearização

Com a dinâmica escrita, vamos criar um drone (com parâmetros arbitrários, mas vocês podem usar os parâmetros reais do drone!) para em seguida criar um modelo linear do drone ao redor de um ponto de equilíbrio. Escolhemos como ponto de equilíbrio o vetor de estados ``X_e``, que representa um drone estacionário na horizontal em uma posição arbitrária.
"""

# ╔═╡ ae331d7b-87be-4bb9-84c5-62d5a505eb5d
bicopter = Bicopter(1000.0, 1.0, 100.0, 40.0)

# ╔═╡ 968dc8b0-f010-4e37-b782-a848cb5d898f
Xe = [
    0   #indifferent
    0   #indifferent
    0   #horizontal
...
# ╔═╡ ce4b62bc-edd6-48c5-9a8f-663c0dec0303
md"""
Para essa posição de equilíbrio, cada motor deve suportar metade do peso do drone. Poderíamos encontrar o vetor de entradas de equilíbrio com um solver numérico, mas é fácil usar a solução analítica nesse caso.
"""

# ╔═╡ 647d7962-2589-4b2f-aa42-b8e452305ceb
#usar roots p generalizar
Ue = 0.5 * bicopter.mass * bicopter.gravity * [
    1.0
    1
]

# ╔═╡ 440d2217-d34f-454d-ba86-e945afaf5be7
md"""
Só verificando que está tudo no equilíbrio.
"""

# ╔═╡ ee289d1d-8f53-43d5-b3b7-8a60cfa59f43
bicopter_dynamics(Xe, Ue, bicopter) == zeros(6)

# ╔═╡ 607c94f6-d003-46d7-915b-8a5cb4fff843
md"""
Agora, a parte preferida de todo mundo. Aquela história sobre jacobianos, derivadas parciais, etc, tudo aqui é necessário para cálculos de controle na vida real. Vamos encontrar as matrizes A e B tais que

``
\dot{X} = A X + B u,
``

lembrando que elas são dadas por

``
A = \frac{\partial \dot{X}_j}{\partial X_i} |_{X = X_e, u = u_e}
``

e

``
B = \frac{\partial \dot{X}_j}{\partial u_i} |_{X = X_e, u = u_e}
``

Felizmente a package ForwardDiff consegue calcular essas derivadas parciais automaticamente. Observem que a matriz A é esparsa (muitos 0s), com algumas caracterísitcas interessantes:
* uma identidade 3x3 no canto superior direito. Isso corresponde ao fato de que as variações de posições e ângulos são dadas por suas respectivas velocidades.
* um termo que vale -100 na posição i = 4, j = 3. Isso corresponde a dizer que ``\dot{x} = -100 \theta.`` A dinâmica de translação depende da rotação do drone! (o que todos vocês já devem saber). Assim, podemos usar o ângulo do drone para causar forças horizontais sobre ele. Por outro lado, a dinâmica de rotação é independente da dinâmica de translação.
"""

# ╔═╡ 7bf3a947-83f9-407b-9697-013e14714c62
A = ForwardDiff.jacobian(X -> bicopter_dynamics(X, Ue, bicopter), Xe)

# ╔═╡ db9be502-f44f-455d-92c0-c68bd808795e
B = ForwardDiff.jacobian(U -> bicopter_dynamics(Xe, U, bicopter), Ue)

# ╔═╡ 7e35058d-b47e-4b67-93b6-b936a08008a0
md"""
Vamos construir o modelo de espaço de estados do drone, como fizemos nas reuniões. Aqui estou usando o ``named\_ss`` para dar nomes a estados, o que não é necessário mas torna as coisas mais claras.
"""

# ╔═╡ f74c28d9-647a-40f9-8ec5-e68182f365d2
linear_model = named_ss(ss(A, B, I, 0), :linear_model,
...
Como vimos numa das primeiras reuniões, é útil poder "solicitar" do hardware forças e momentos específicos sem se preocupar com cada motor individual. Esse é o trabalho da HAL, que pode ser descrita por uma simples matriz tal que

``
\begin{bmatrix} 
	T_{esq} \\ T_{dir}
\end{bmatrix} = HAL \begin{bmatrix} F \\ M \end{bmatrix}
``
"""
...
md"""
# Ajuste de ganhos de θ

Como previamente mencionado, a dinâmica rotacional não depende da dinâmica translacional, de modo que podemos ajustar os ganhos dela usando um sistema simplificado. Na célula abaixo colocamos a HAL em série com o modelo linear, extraimos o sub-sistema com entrada momento e saídas ângulo e velocidade angular.
"""
...
md"""
Podemos agora impor alguns requisitos como tempo de resposta tᵣ (para 5% de erro) e amortecimento δ para fazer alocação de pólos. O amortecimento escolhido foi 0.7 pois é o menor amortecimento para o qual não há ressonância (não é o único critério possível)
"""

# ╔═╡ de8d73a3-54f4-473d-8317-cecd75d91947
tr = 1 #s
...
md"""
Fórmula aproximada pra frequência natural do sistema com base no tempo de resposta e amortecimento
"""
...
md"""
Cálculo da matriz de ganhos necessária para que o sistema tenha os polos desejados. Como nosso sistema tem 2 saídas, temos 2 ganhos.
"""

# ╔═╡ e4b9e919-a5ef-4d90-abde-abedde4a60ce
Ktheta = place(theta_dynsys, desired_poles) .|> real
...
md"""
### Verificações
* Realmente obtivemos os polos que queríamos;
* O tempo de resposta de fato é 1s;
* O sistema está bem amortecido (não há comportamento oscilatório)
"""
...
Agora, vamos dar maior atenção à rejeição de perturbações, para simular um drone com massa desconhecida. Vamos adicionar uma entrada ao sistema para representar uma força perturbativa em y, e sem seguida ajustar os ganhos PID com base nos gráficos de resposta degrau na referência y e na perturbação y. 
...
md"""
# Construção do sistema final

Agora podemos juntar tudo em um grande modelo de espaço de estados que simula nossa dinâmica linear para investigar o comportamento de cada saída do sistema com relação a cada entrada, por exemplo.
"""

# ╔═╡ 0f0f8d60-f790-4883-86e5-e77fedcd9aa4
position_control_layer = append(xpid,  ypid);

# ╔═╡ ea6e516f-1a1f-4105-b22b-73b4860fd560
...
md"""
## Exemplo: evolução de θ para um degrau de posição x.
"""
...
Plots = "~1.39.0"
PlutoUI = "~0.7.52"
RobustAndOptimalControl = "~0.4.28"
"""
...
git-tree-sha1 = "0133bca0dcac6b007905ebe827322bc9a551989f"
...
git-tree-sha1 = "db640af8b65836e3f7cd1b8d9848a4e24830cd6e"
...
git-tree-sha1 = "3783f61707e5b9d1c45c4ebff0b1745dd91f3cdb"
uuid = "99c1a7ee-ab34-5fd5-8076-27c950a045f4"
version = "2.2.11"

[[deps.MatrixPencils]]
deps = ["LinearAlgebra", "Polynomials", "Random"]
git-tree-sha1 = "6e46ed91749d2bbd110e25aa07086ab4b99cb145"
...
git-tree-sha1 = "35882aa39079c44ea6c1f07827f98dc78bf1ce91"
...
git-tree-sha1 = "2ad483abffc0244d9096f0e316fc3d4c1749c666"
